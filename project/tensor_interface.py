...
def st_select_index(tensor_shape, n_cols=3):
    oi = [0] * len(tensor_shape)
    cols = st.columns(n_cols)
    for idx, dim in enumerate(tensor_shape):
        oi[idx] = cols[idx % n_cols].number_input(
...
def interface_visualize_tensor(tensor: Tensor, hide_function_defs: bool):
    ...
    oi = [0] * len(tensor.shape)
    to_index(selected_position, tensor.shape, oi)
    st.write(f"**Corresponding index:** {oi}")
    st_visualize_tensor(tensor, oi, show_value=False)
...
def interface_permute(tensor: Tensor, hide_function_defs: bool):
    ...
    if viz_type == "Original tensor":
        ...
        viz_tensor = p_tensor
    st_visualize_tensor(viz_tensor, oi, show_value=False)
    st_visualize_storage(
        tensor, index_to_position(oi, viz_tensor._tensor.strides)
    )
